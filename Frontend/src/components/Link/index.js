...
const Links = props => {
  ...
  const { phase, brand, page, finalUrl, fetchTimeStart, fetchTimeEnd } = searchParams(
  ...
  return (
    <Link
      ...
      to={{
        pathname: to,
        search: arr.includes(to)
          ? setSearch({ phase, brand, page, ...getDefaultDateRange() })
          : to === '/gatling'
          ? search({ phase, brand, fetchTimeStart, fetchTimeEnd })
          : ''
      }}
    >
      {children}
