...
def t_fun(file):
    ...
    for Audit in Audit_list:
        for temp in Audit_list[Audit]:
            ...
            Audits[Audit][temp1] = dict()
            try:
                Audits[Audit][temp1]['score'] = (data['audits'][temp]['score'])+(data['audits'][temp]['score'])*(random.randint(0,30)/100)
            except:
                pass
            try:
                Audits[Audit][temp1]['scoreDisplayMode'] = data['audits'][temp]['scoreDisplayMode']
        ...
        for t in t_list:
            try:
                ...
                pass
            try:
                score =data['categories'][weight_dict[Audit]]['score']+ data['categories'][weight_dict[Audit]]['score']*(random.randint(0,30)/100)
                Audits[Audit]['score'] = score
            except:
                pass
    audits['seo_audits']=Audits["SEAudit_list"]
